...
class TelegramOpenidConnectImpl(
    ...
    override suspend fun validate(
        ...
        val telegramOIDCProfile = runCatching {
            createTelegramOIDCProfile(response.idToken, telegramPublicKey)
        }.getOrElse { failure ->
            logger.warn(failure) { "Cannot create TelegramOIDCProfile from idToken from telegram response: '$responseString'" }
            return ValidateTelegramOpenidConnectResult.InvalidAuthorizationCode
        }
    ...
    private fun createTelegramOIDCProfile(idToken: String, publicKey: PublicKey): TelegramOIDCProfile {
...
private fun createCodeChallenge(codeVerifier: String): String {
    ...
    val result = java.util.Base64.getUrlEncoder().withoutPadding().encodeToString(digest)
    return result
}

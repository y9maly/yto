...
internal class PostgresFileRepository(private val main: MainRepository) : FileRepository {
    override suspend fun exists(id: FileId): Boolean = main.transaction(ReadOnly) {
        TFile.select(intLiteral(1))
            .where { TFile.id eq id.long }
            .count() > 0
    }

    override suspend fun totalUserStorageUsed(client: ClientId): Long? = main.transaction(ReadOnly) {
    ...
    override suspend fun get(id: FileId): File? = main.transaction(ReadOnly) {
    ...
    override suspend fun getImageFile(file: FileId): ImageFile? = main.transaction(ReadOnly) {
    ...
    override suspend fun getFileTypes(file: FileId): FileTypes? = main.transaction(ReadOnly) {
